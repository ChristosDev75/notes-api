...
Module: app.main
Purpose: FastAPI application definition and HTTP endpoint
         orchestration
...
---------------------
This module serves as the primary container for the Notes API
system. It defines the FastAPI application instance and all HTTP
endpoints that external clients interact with. This is the boundary
between the outside world and our internal business logic.

C4 MODEL MAPPING
----------------
@c4-container: API Application
@c4-technology: Python 3.12, FastAPI 0.104
@c4-description: REST API providing CRUD operations for notes with
                 automatic request validation and OpenAPI
...
1. **Framework Choice: FastAPI**
   ...
   - Rationale: Type hints improve IDE support and catch errors
                early
   - Trade-off: Slightly higher memory footprint vs Flask
   - Alternative Considered: Flask + Marshmallow (rejected due
...
2. **Lifespan Management**
   ...
   - Future Use: Could add connection pool warmup, cache init
...
- 422 Unprocessable Entity: Invalid request data (handled by
                            FastAPI/Pydantic)
- 500 Internal Server Error: Unexpected exceptions (FastAPI
                             default handler)
...
async def lifespan(app: FastAPI):
    ...
    Manages the complete lifecycle of the FastAPI application from
    startup through shutdown. This pattern ensures proper resource
    ...
    - Any exceptions during startup will prevent the app from
      starting
...
app = FastAPI(
    ...
    lifespan=lifespan,
    description=(
        "A simple REST API for managing text notes "
...
def read_root():
    ...
    Provides a simple health check endpoint to verify the API is
    running. Useful for monitoring, load balancer health checks,
    and quick status verification.
...
def create_note(note: NoteCreate, db: Session = Depends(get_db)):
    ...
    Creates a new note with the provided title and content. The note
    is immediately persisted to the database and assigned a unique
    ID.

    WORKFLOW
    --------
    1. FastAPI validates request body against NoteCreate schema
       (Pydantic)
    ...
    HTTP Status:
        ...
        422 Unprocessable Entity: Invalid request data (automatic
                                  from FastAPI)
...
def get_notes(skip: int = 0, limit: int = 100,
    ...
    Retrieves a paginated list of notes from the database. Supports
    basic pagination via skip/limit parameters.

    PAGINATION STRATEGY
    -------------------
    - Default: Returns first 100 notes
    - Max Limit: 100 notes per request (prevents excessive memory
                 usage)
    - Skip: Offset for pagination (e.g., skip=100 for second page)

    Note: This is a simple offset-based pagination. For large
    datasets, cursor-based pagination would be more efficient.
...
def get_note(note_id: int, db: Session = Depends(get_db)):
    ...
    Retrieves a specific note by its unique ID. Returns 404 if the
    note doesn't exist.
    ...
    HTTP Status:
        ...
        422 Unprocessable Entity: Invalid note_id format (not an
                                  integer)
...
def delete_note(note_id: int, db: Session = Depends(get_db)):
    ...
    Permanently deletes a note from the database. This is a hard
    delete - the data is immediately removed and cannot be
    recovered.
    ...
    Future Enhancement: Implement soft delete with 'archived' flag
    to allow recovery and maintain audit trail.
